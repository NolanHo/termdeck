...
test('mcp server exposes termdeck tools and can step through daemon', async () => {
  ...
  try {
    ...
    for (const name of ['list_sessions', 'new_session', 'step', 'project_step', 'run', 'poll', 'state', 'summary', 'last_command', 'scrollback', 'transcript', 'expect', 'expect_prompt', 'send_input', 'paste', 'ctrl', 'signal', 'kill', 'task_start', 'task_status', 'task_recover', 'task_logs', 'task_dashboard', 'task_prune', 'task_stop']) {
